...
train_path = "C:\\Users\\Naimish\\OneDrive\\Desktop\\TrainData.csv"
...
def clean_data(path,data=pd.DataFrame()):
    ...
    for i in range(len(data['review'])):
        string1 = ''.join(letter for letter in data['review'].iloc[i] if letter not in ['.', ',', ':', '@', '[', '!', ']', '=', '&', '(', ')', '?', '"', '#', '$','{', '}', '\\', '/', '%'])
        if 'train' in path:
            data['review'].iloc[i]=count(string1)
        else:
            words = set(string1.split())
            words=words-set(stopwords.words('english'))
...
randomforest = GridSearchCV(model,{'n_estimators' : [450],'min_samples_split' : [6],'max_features' : [0.4]},n_jobs=True,return_train_score=False,verbose=True)
